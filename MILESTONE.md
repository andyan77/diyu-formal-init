...
- 状态：`REVIEW`（2026-07-26 静态产品合同与四条全链路候选完成，等待创始人逐旅程审阅）
- 前置：M7-3、UI-01R 已 `CLOSED`；UI-01 保持 `SUPERSEDED`。UI-01 与 UI-01R 历史目录
  完整保留，不改写成正式 React 复制模板。
- Git 基线：启动时本地与 `origin/main` 均为
  `f69380b40e891ea3597cc873751f6a4e9d2a721b`，工作树干净。
- 唯一执行端：当前 WSL 执行端；同一时间只允许一个写入者。
- 当前交付：三套相互隔离的产品空间合同、四条可连续浏览的任务旅程、租户管理完整旅程、
  总部/区域/代理商/门店共用的统一创作壳、纯文字 DM01、25 项精简 V1 能力消费者与黄金证据
  映射，以及 UI-G/CAP-G 黄金合同；评审工具继续与裸产品页面分离。
- 首页裁决：正式方向采用 A“种子找到声音”；B/C 只保留为历史候选。A 在本轮减少路径演示感，
  朱砂只承担一次方向转折，最终正式 Logo 留有约 `0.8—1s` 静止呼吸；减少动效时直接显示
  正式 Logo 和首页。
  首页文字仍为产品候选，不冻结为永久品牌口号。
- 产品架构：工程上只保留租户管理端、统一创作端、笛语运维端三套隔离空间。总部、区域、
  代理商和门店不复制前端，差异只来自可信组织、当前发布账号、ContentRole、操作者资格和
  当前可用能力；双入口资格必须显式切换。
- DM01 边界：第一阶段只交付完整文字参考方案，可表达左右、上下杆、正/侧挂、主次、数量、
  间距、替代办法和执行顺序；不提供或暗示陈列示意图、效果图、墙面预览或自动还原现场，
  不标 AIGC，也不要求确认、授权、审批或采用证明。
- 写入边界：只修改文档、只读能力映射和离线静态原型；不修改正式 React、服务端、API、
  OpenAPI、数据库、认证、权限、生产配置、ECS 或生产数据，不部署、不调用 DeepSeek、
  不触发远程 CI。
- 有界验证：7 张裸产品/评审 HTML 的本地引用与 HTML/JS 基础解析通过；真实 Chrome 对 30 个
  产品状态分别以 `1440×900`、`390×844` 完成 60 次视口核对，横向溢出、多个实心主动作、
  小于 `44px` 的可见触控目标、内部词、控制台错误和外部请求均为 `0`。四条旅程连续走通；
  内容与纯文字 DM01 均成立 `V1→V2→V1`，DM01 图形元素与 AIGC 标记均为 `0`；A 动效约
  `7.28s` 完成，跳过和减少动效成立。精简目录机械对账为默认 `21`、体型主动启用 `4`，
  45 个非稳定 ID 去向为 `F10 41 / F03 1 / F07 1 / F08 1 / F06 1`。
- 后继边界：`UI-03` 才承接正式 React 设计系统、公共首页、A 动效、三入口、统一创作端和
  内容主链，但不切换生产默认 UI；`UI-04` 才接入纯文字 DM01、系列、素材、租户管理、运维、
  状态页及生产切换与回退。当前不启动 UI-03。
- 唯一下一动作：创始人逐旅程审阅 UI-02；不得自行 `CLOSED`，不得启动 UI-03。
