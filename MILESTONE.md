...
- 状态：`ACTIVE`（2026-07-26 启动；当前只做跨里程碑接缝验收，不新增功能）
- 前置：M7-1、M7-2A、M7-2B 均已 `CLOSED`；M6-3 保持 `SUPERSEDED`，不恢复候选、
  不重跑 Q1—Q8。
- Git 基线：启动时本地、`origin/main` 与生产部署仓库均为
  `1e4423519ec74e69d062f3eb6809c06901921590`，工作树干净。
- 生产锚点：运行/唯一 CI 标签
  `7f920d491d198b2f9a4ef7d9565f9c7af686d300`，镜像摘要
  `sha256:64c0ec711e9daaa9d5d585578849ae64dc903f4afe0bb40de6034147d283b266`，
  schema `20260730_24`，CI run `30194854609` 为 `success`，回环与公网 readiness 正常。
- 唯一执行端：当前 WSL 执行端；同一时间只允许一个写入者。
- 当前任务包：`docs/M7-3-第一阶段整体验收与停止执行包.md`
- 当前任务：复用 CLOSED 里程碑的不可变证据，只新鲜验证内容生产/DM01、三入口、桌面/移动、
  画像/版本/偏好隔离、同品牌差异、三平台、能力目录、反馈候选和首期以后去向之间的接缝。
- 代码与生产纪律：默认不改代码；若无真实集成缺陷，不部署、不迁移、不建备份、不触发 CI。
- 当前剩余验收：定向与唯一成功完整本地门已通过；只剩唯一远程 CI、有界备份、同 SHA 部署、
  桌面/移动生产复验、最终生产锚点与权威文档收口。
- 唯一下一动作：完成唯一候选 CI 和必要生产复验后将 M7-3 置为 `REVIEW`；不得自行
  `CLOSED`，不得启动第一阶段以后施工。
