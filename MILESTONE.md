...
- 状态：`ACTIVE`（2026-07-26 主控裁决 UI-02 有限范围通过后启动）
...
- 当前任务：实现正式 React 公共首页与 A 动效、三套隔离产品空间的入口壳，以及统一创作端
  的正式内容主链；复用服务端可信会话、五轴目录、内容任务、自然修改、历史版本、复制导出、
  素材选择和私人默认，不建设第二套认证、权限或内容业务。
...
- 能力边界：目录继续由服务端版本化接口提供，默认可见 21 项，4 项体型相关方向仅在本人主动
  ...
  资产，不补齐 39 项缺口。
- 生产边界：本里程碑不部署、不切换生产默认 UI，不修改 ECS、生产数据库、Nginx、DNS、
  生产配置或生产内容；不创建生产账号或凭据。候选稳定后只触发一次远程 CI。
- 当前剩余验收：正式 React 的首页与三入口；V1→V2→V1、复制/导出；五轴、自定义、个人默认、
  体型主动启用与透明转译；总部/门店同壳与可信切换；桌面/移动/键盘/触控；两份有界审查；
  本地完整门与唯一 CI。
- 唯一下一动作：当前执行端连续完成 UI-03 到 `REVIEW`；不得部署、不得启动 UI-04。
