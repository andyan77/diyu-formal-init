...
- 状态：`ACTIVE`
- 起点：M7-1 已 `CLOSED`；施工基线为 `07fd4b8a54c1ad96fc237b97f18cff0cdb101847`，
  当前生产运行实现仍为 `ac6e03c3b3a7e8fd8a768884746d0cb8a1d0cd59`，schema `20260725_19`，
  指定上一回退目标 `1241cbd04aee8d3e28637d13323ae17d657c7082`。
...
- 边界：本里程碑只交付最小内容控制面（版本化五轴目录、实际启用发布账号的五段人话画像、
  ...
  计划、未满足需求候选）。不启动 M7-2B，不做真实连续系列、真实商品 P1/P2/P5、三平台重
  编译验证、平台流量或内容市场结果。
- 唯一下一动作：按 M7-2A 执行包完成八个闭环、确定性验收、一次有界真实模型冒烟、F11 有界
  清理与一次生产部署，然后置 `REVIEW` 等待主控终审。
