...
- 状态：`ACTIVE`（2026-07-26 主控终审退回，仅关闭产品壳无来源断言和内容默认目标两个接缝）
...
- 承重实现：`f27e996e274ce8d360a6876e501d8fa6c01a0e57`；唯一远程 CI
  `30234295641` 为 `success`。最终状态文档提交见本里程碑收口提交。
...
- 当前剩余验收：删除管理/运维壳无来源状态断言；无查询参数进入 `/content` 时，由服务端从
  正式会话合法目标中解析稳定默认值，并保持显式未授权目标失败关闭；完成定向验证、完整门、
  两份有界审查和唯一远程 CI 后重新进入 `REVIEW`。
- 唯一下一动作：当前执行端完成上述两个极小接缝；不得部署、不得启动 UI-04。
