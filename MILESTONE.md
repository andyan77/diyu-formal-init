...
- Git 基线：启动时本地与 `origin/main` 均为
  `872c657a54e37abf90fb709b512c9b17d404e821`；当前工程/生产候选均为
  `0ec4964d6e9c6a2f6daa5a420b5268510347ddf6`，本地与生产部署仓库干净。
- 唯一执行端：当前 WSL 执行端；同一时间只允许一个写入者。
- 当前任务包：`docs/M7-2B-真实品牌内容协作试点执行包.md`
- 当前生产（2026-07-25 只读核验）：部署仓库与运行镜像均为
  `436ac8923e45263a687ee8d2ce0b758371b1fb08`，镜像摘要
  `sha256:81cbada1c468fc6cc59c340225fd33afed4919231cee9c5e1c1b46f45373413c`；
  schema `20260727_21`；本机与公网 readiness 正常，公网首页 `303`；部署仓库干净。
- 当前真实资料缺口（只读现场）：笛语服饰只有一个抖音发布账号，控制组织来源为
  `inferred`；该账号五段画像 `0`、其操作者画像维护授权为 `false`；真实长期商品 `0`、
  连续系列 `0`。生产另有“浙江分公司”组织，但没有可冒充真实门店人物的启用账号。
- 当前边界：不依赖真实资料的连续系列、明确平台载体、真实商品最小事实入口和自然用户
  界面薄连接已经完成；渐进入驻减负补口进一步把总部五段画像、三件 V 级商品候选和缺失平台
  内容载体预填到管理页，全部保持“确认前不是事实”，`inferred` 仍不授权，平台不连接外部。
  本轮补口本地门为后端 `143 passed`，前端内容/管理端交互均通过；最终同 SHA CI
  `30182867956` 绿色，生产已部署该 SHA，schema `20260728_22`，镜像摘要
  `sha256:9de6f421dd530de52328007a4e9ebf2388e2e13687ea609b63f92167aa24006f`，
  新鲜备份 `20260726T012805Z-predeploy` 可读，本机/公网 readiness 正常。上一回退镜像
  `52a70348255f64cce6efbe5c082542ecadd2180e` 保留。现在进入唯一一次
  合并真实资料动作。六篇源成品和
  六份平台版本必须走正式 DeepSeek；不创建临时自然人，不接 DIFY，不建第二模型、裁判
  模型、审核 Agent、自动发布、实时爬虫、ERP、设置页或第二套状态机。
- 剩余验收：两个真实身份与画像、两组各三篇连续系列、两条 V1→V2、真实商品 P1/P2/P5、
  两个源成品的三平台版本、一次透明品牌转译、两份有界人工审查、完整本地门、唯一 CI、
  新鲜备份、同 SHA 生产部署与健康/隔离/回退证据。
- 唯一下一动作：笛语服饰有权主体只在生产租户管理页纠正并确认已预填的总部控制组织/画像、
  1—3 件商品和必要内部平台载体；真实门店人物本人另行完成身份、一次激活、画像和一句真实
  种子。完成后执行端立即继续两个哨兵篇。不得启动 M7-3。
