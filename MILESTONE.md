...
- 第 0 步：只把已核实的 17 条责任合同运行／测试路径以前向修改恢复到 `845f632…` 的
  运行内容；不 reset、rebase、squash，不回退文档、迁移、认证、配置或部署文件。恢复提交
  与上述两条本地文档提交一次推送，且只触发一次恢复基线 CI；恢复提交不部署。远端新 HEAD
...
- 唯一下一动作：**完成 17 路径前向安全恢复与恢复基线 CI，然后在该安全基线上实现并验证
  UI-06**。持续到 `UI-06 REVIEW` 或命中已冻结结构停止线；不得形成 UI-05 候选 C，
  不得重新部署候选 B。
