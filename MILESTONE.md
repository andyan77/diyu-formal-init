...
- 当前里程碑：`BRAND-MATRIX-01 · 十账号品牌知识与八剧本全能力演示闭环`。
- 状态：`IN-PROGRESS`（2026-08-07 立项）。founder 一揽子裁决：D01—D14 全部按推荐值放行
  （原话「全部授权」），唯一附加条款——26 条云盘视频品牌标识须全部统一为「笛语／DIYU」
  后方可正式使用（口径锚定《笛语素材品牌标识统一任务表 V1》）。实施结构 S0 + Gate A—E，
  Gate A—D 不触生产、最终唯一候选一次部署；裁决全文、素材路线修正与 S0 悬账清单见
  `docs/COMM-01-执行包排产与工程对照指南.md` §六。
- COMM-01 据 D10 转入部分暂停：第一部分（EXE-01R/EXE-V0/EXE-V1）交付保留，EXE-V1
  终局监理复验并入 S0；第二部分（EXE-02—EXE-12）暂停，其直接消费者收编入本主线；
  不创建平行状态文件，下方 COMM-01 登记按历史快照原样保留。
...
- 该快照只读记录的生产运行实现为
  `f77a2e852758b9157425633041aea59d9f2b24da`；当时的运行镜像 digest 为
  `sha256:f869aa605f3bc81a429a56a4081b1c4eabaa351587245f5b8aa73fb19f7bb683`，schema
  `20260813_40`，公网／回环 readiness、liveness 与 `/status` 均为 200。当时的唯一权威 CI
  `30712642287` 只承重该历史生产候选；该轮未 push、未触发 CI、未备份或部署。当时已有的部署前备份
  `/var/backups/diyu-m5-4/20260801T183351Z-predeploy` 已通过权限、checksum、隔离恢复、
  FORCE RLS、应用 readiness 与对象恢复；上一健康镜像在新 schema 上完成回退往返。
- 该快照记录笛语服饰生产中已有 21 份源文档、5,046 个稳定语义 segment、14 个品牌授权候选
  商品与 203 条字段证据，其中 26 个 V 级字段进入 ProductFact；当时正式门店事实、正式商品媒体、
  门店档案和库存仍为空，因而 P4、P5、DM01 均以 `data_missing` 记录资料缺口。当时的隔离
  黄金套件与人工全文审阅证据位于
  `/var/lib/diyu-tenant01-evidence/96dcb74d4538f8f929193292b119333a64ee9558/golden-v1/`。
- 该快照的软件功能真值以 `58/0/0/6/0` 为基线，必须与笛语服饰资料就绪度分开；资产为
  `41/243/25/119`、激活增量 0，除非该日期的现场正式事实要求另行诚实记录。
- 该快照的共享内容语义实现基线为 `e54660266efee3398b93502d5755260c54248f85`：固定服务端 P1
  成稿层已删除，Writer brief 已停止消费全句 claim／句式约束，Compiler 不再生成创意观点；
  证据回放也已改为以完整冻结 ProductFact 注册表校验本次实际选择的事实块。`deepseek.py`
  由失败基线 `0fa0636…` 的 4,920 行降至 4,694 行，`server_bearing_expression.py` 由
  287 行删除，`account_editorial_lens.py` 由 260 行降至 200 行。当时的验收与证据实现
  `8055dfbd0d861b73aa1fa4d6f14d910566072813` 没有改动内容语义路径，已把 11 卡与 15 卡冻结
  泛化结果绑定到上述 `26/26 + 26/26 + 至少 23/26` 门，并拒绝预填 PASS、摘要漂移和硬边界
  失败后的 V2 洗白；它在该快照中仍是本地 WIP，未成为运行候选。

> **以上为 2026-08-03 返工前历史快照；后续条目继续按发生时间保留。**

- `0286749…`、`48d8249…`、`49ee04a…`、`0fa0636…`、`714558a…` 与 `9da7432…` 的失败候选均
  原样保留。用户裁决“条件式开放 Writer”后执行了两轮新的完整三卡：`714558a…` 中 P2 通过，
  P1 将冻结前提改写为用户经历并补充服装类别效果，daily 新增身体／心理／因果解释；`9da7432…`
...
- 该轮 Gate D 本地候选继续使用 schema `20260810_37`。新 React/API 以稳定商品版本 ID 与
...
- 该轮本地候选的 64 项功能真值为 `58/0/0/6/0`；没有继续标为有缺陷、占位或尚无法证明的正式项。
...
- 冻结商品引用现只接受有明确自然语法边界的本次 SKU 原值或完整商品名称；casefold 仅用于
  唯一字面别名比较，解析结果始终返回数据库 SKU 原值。`abc-1234`、`Xabc-123`、`ABC123X`
  及商品名称升级款均失败关闭；未知／歧义硬约束和修订目标在任务或新 run/version 创建前
  自然澄清，差分保持 `0/0/0`。恢复无边界子串命中的 mutation 会使对应反证真实变红。
- 该轮历史状态为 UX-03 `REVIEW`、Gate D `COMPLETE / 等待主控最终定向终审`。冻结商品引用现为
...
## TENANT-01 历史 REVIEW 交付（2026-08-04，运行实现 `748190c…`，已被取代）

> 本节只记录上一健康运行实现 `748190c…` 的追加式历史事实；当前生产、当前证据和当前 REVIEW
> 结论以紧随本节之后的 `c5eb588…` 最终交付为准。
...
- `docs/项目记忆.md` 未暂存、未提交、未覆盖，最终 diff SHA-256 保持
  ...
  仍按禁止改写历史约束如实披露。
- 该历史候选当时未证明正式租户真实内容生产与真实发布、真实员工采用、平台流量／排名／爆款、GMV／
...
- 笛语服饰正式发布投影为 confirmed V2：id `402278fb-15b6-4739-b947-32b38beb917e`、digest
  ...
  segment、版本与 digest；兼容基线 V1 追加式保留。19 份授权资料与 2 份模板均在覆盖矩阵有
  明确去向，模板、目录、验收问题、内部脚手架与用户排除的 26 条视频不进入 Writer；商品字段只
  经独立 ProductFact 管道。5,046 个 segment 是证据库，不以“全部塞入 Prompt”冒充可消费。
- 正式任务冻结并实际消费 projection V2 中与 `brand_life_narrative` 相符的 4 项来源引用；
  candidate freeze、WriterRequest、task snapshot、context evidence 与最终生产证据的 projection
  id／version／digest 完全一致。后续投影不会污染旧任务，V2 重放原冻结上下文。
- 正式“笛语品控”输入“今天去工厂验厂，今年量装大货的车缝品质有了大幅度的提升”，完成
  V1→V2→V1→V2、复制和导出；task／run／version 差分 `+1/+2/+2`，succeeded run 为 2，
  永久 running 为 0。服务端冻结原句不变，Writer／ProductFact refs 均为 0；线迹、针脚、接缝、
  收边等非量化常见维度和低风险场景只属于 `creative_expression`。V1/V2 高风险边界、结构与首稿
  可用均为 `PASS`，V2 有真实但较小的可见差异。
- 明确商品输入继续只加载所选 SKU 的已确认 ProductFact；无来源机构保证与未登记 SKU 在任务和
  Writer 前失败关闭。量化规格、检测／认证、合格率、提升百分比、具体 SKU／全批次结论、机构
  保证和未提供的工艺／设备／材料／性能／功效／体验不能借创作表达进入可信事实域。
- 正式用户从 2 增为 3：“柯桥店阿丹”以正式组织和全局唯一中文用户名建立，display_name 允许
  与 `legacy_hidden` 历史身份同名；历史 UUID、6 个任务和授权不变。重复 username 返回
  `USERNAME_TAKEN` 且零部分写；权限 403 保留会话，只有真实认证失效才回登录。
- P4 因正式门店事实为 0、P5 因正式组织媒体和商品媒体绑定为 0、DM01 因正式门店和库存为 0，
  均继续为 `data_missing`；普通生活、感悟、抱怨和工作现场内容不受这些局部缺口阻断。系统不
  自动发布，采用和发布由用户完成。
- 新鲜 predeploy 备份位于
  `/var/backups/diyu-m5-4/20260805T051600Z-tenant01-c5eb588844f9d398f742aa8a5406e4c5f41900bc-predeploy`；
  0700／0600、checksum、隔离恢复、FORCE RLS、无作用域读取拒绝、对象恢复和 readiness 均
  `PASS`。上一健康 `748190c…` 镜像在 schema 44 上完成六端点与正式历史读取，再切回 c5eb；
  数据库未降级，正式内容／陈列历史指纹前后一致。
- 清理完成：本地 15 个精确匹配的正式纵向 synthetic tenant 及其关联行归零；生产验收 session、
...
- 2026-08-09：用户明确授权删除 56 个未挂载匿名 Docker 卷；冻结删除集 digest `ebe0a7b7…90cbe`，`56/56` 删除成功，卷总数 `62→6`（保留 6 个全部在用），容器快照与当前镜像不变，live/ready 均 200；可用空间约 `3.18GB→6.62GB`。**构建容量前置阻断已解除，Gate E 仍为 ACTIVE、尚未冻结**；SEALED-SET-01 未开封、模型请求 0、累计 59/300。唯一下一动作：恢复 E-1 build-once 与六对象冻结。
- 2026-08-09：Gate E E-1 已冻结六对象：runtime candidate `65995f3ba6ba99f79941395ab5042f671d2bc2e7`；build-once 镜像 `sha256:ec48710d…e0b1`；Writer Prompt、模型、守卫/三层制和数据 manifest 包摘要均登记。候选未运行，线上旧镜像与 200/200 健康状态不变；未部署、迁移、导入、上传或切账号。状态：**`E-1 FROZEN · AWAITING_CONTROLLER_OPEN`**；SEALED-SET-01 未开封，Gate E provider request 0、累计 59/300。唯一下一动作：等待主控开封并逐字提供 20 卡明文。
- 2026-08-09：Gate E E-2 密封盲测在冻结候选上逐字起跑：B01 机器硬判 `12/12 PASS`；B02 的品类/主色 V 级引用、事实不改值、组织可见性与通用禁区均通过，但未满足专属硬判“未确认项须显式声明不作判断”（`S-02-UNKNOWN-DISCLOSED`）。依 Prompt 6 首败即停，B03—B20 请求 0；本轮请求 2、传输重试 0、累计 `61/300`。状态：**`GATE-E FAILED_SAFE · SEALED_BLIND_MACHINE_GATE_FAILED`**（待监理复验）；未生成盲测成品包、未评分、未部署，冻结后代码/Prompt/规则/fixture/数据改动 0。唯一下一动作：主控/监理复核并签发后续安排，执行侧不补跑。
- 2026-08-09：主控续跑令 rev2 以 `ADJ-BLIND-ORACLE-01` 将 B02“未知项显式声明”移入人工评分，保留原输出并授权全量续跑。20 卡现已全部产出或形成正式前置问题态，机器 `13/20 PASS`；失败卡 B04/B10/B11/B12/B16/B19/B20。Gate E 有效内容响应 17，B11/B12/B16 各 0（provider 前阻断），传输重试 1，累计 `76/300`；零重写、零择优、零拼接。完整集中评审包已落盘。状态：**`E-2 COMPLETE · AWAITING_CENTRAL_REVIEW`**，但机器 20/20 硬门未过，生产部署保持 BLOCKED。唯一下一动作：监理集中初裁 + founder/监理人工评分；双硬门齐过前不部署。
...
- 2026-08-09 · R-1 监理复验 PASS（独立门禁全绿 1061/2、EXE-01 9/9、CI 四查过），e0f520a 合入主线；《问题诊断与里程碑修复部署方案 v2》交付待审查方审查；下一步 E-1' 重冻结+考务合同 v2+SEALED-SET-02。
- 2026-08-09 · 方案 v2.1 落库（审查方六项补强全采纳：考务先行入冻结八对象、三卡回归硬前置、负向套件分离、四门恢复、oracle 审计制度、三处口径再勘正）；E-1' 按新序重签待发。
- 2026-08-09 · Gate E E-1' 考务先行冻结包：考务合同/执行工具/oracle/负向套件已落盘，readiness `3/3`、负向污染差分 `0/0/0`；全量 `1068 passed / 2 skipped` 与 CI `31332774814` 四查全绿；候选 `bb9e63d…3266` 及八对象已一次性冻结，build-once 镜像 `sha256:4ba10759…91be`。Phase D 开始前仅检查进程环境发现三个 `DEEPSEEK_*` 键均未注入，且本包禁止读取 `.env`，故模型请求 0、累计仍 `76/300`。状态：**`PRECONDITION_BLOCKED · PROVIDER_CREDENTIALS_NOT_INJECTED`**；生产接触 0。唯一下一动作：主控以进程环境精确注入三键后，在同一冻结候选上执行 B11/B12/B16 各一次公开回归。
