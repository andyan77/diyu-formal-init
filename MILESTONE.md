...
- 状态：`ACTIVE`（2026-07-26 主控终审退回；仅关闭 `current_target` 与正式页面登录恢复两个
  产品接缝，不重开其他验收面）
- 前置：M7-1、M7-2A、M7-2B 均已 `CLOSED`；M6-3 保持 `SUPERSEDED`，不恢复候选、
  不重跑 Q1—Q8。
- Git 基线：启动时本地、`origin/main` 与生产部署仓库均为
  `1e4423519ec74e69d062f3eb6809c06901921590`，状态检查点为 `8f82864`；最终承重实现、唯一 CI
  与部署 SHA 为 `9f987e3a035563e9cf3eb8f676d27c55e0688e52`。
- 本轮返工基线：主控终审退回时本地与 `origin/main` 为
  `5d41b2c81bf083d2f8cc53a20d6f23a9a6b9786b`；生产继续运行上一承重实现，不回退。
- 生产锚点：运行镜像摘要
  `sha256:a8fdc56d2f0e4600d97be10b3afb470099f7a22cebbb9e01f8045fb03625bddf`，
  schema `20260730_24`，唯一 CI run `30198339049` 对应上述 SHA 且为 `success`；回环与公网
  readiness 均为 `ready`。部署前备份
  `/var/backups/diyu-m5-4/20260726T103410Z-predeploy` 的校验全部通过；上一运行镜像
  `7f920d4…` 及摘要 `sha256:64c0ec…` 保留为回退锚点，不执行 schema downgrade。
...
- 当前剩余验收：服务端 `/content` bootstrap 显式返回已解析的 `current_target`，前端以它
  初始化并在不手工切换时原样提交；正式用户页面匿名访问使用入口对应的 `303`，错误入口资格
  返回带恢复动作的人话 HTML `403`，`/api/v1/*` 继续维持 JSON `401/403`。其余已通过结论
  不重开；既有未证明项保持不变。
- 唯一下一动作：完成上述两个接缝的定向测试、唯一完整门、唯一 CI、备份、同 SHA 部署和生产
  复验后重新进入 `REVIEW`；不得自行 `CLOSED`，不得启动第一阶段以后施工。
