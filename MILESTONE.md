...
- 当前里程碑：`UI-06` 自然创作与叙事真实性闭环。
- 状态：`ACTIVE`。`UI-05` 已在本次前向状态交接中置为
  `SUPERSEDED → UI-06`；这只表示被否决的自报事实标签／单元修复路径由 UI-06 的类型化叙事
  闭环替代，不表示 UI-06 已成功。UI-05 的 G3/G4/G7 失败、候选 A/B、回退与停止线证据全部
  原样保留。
- 启动现场：本地启动提交
  ...
  工作树干净。生产继续运行健康的
  `845f63291ba5060e60f87d1afa5cfc1cdb057e3b`，不合格候选 B 不在生产运行。
- 第 0 步已完成：只把已核实的 17 条责任合同运行／测试路径以前向修改恢复到
  `845f632…` 的运行内容；不 reset、rebase、squash，不回退文档、迁移、认证、配置或部署
  文件。恢复与状态交接提交为
  `dc86b9520001a91e6b3d37591baa068f8f8b05ea`，与 `31cec35…`、`4b27f2f…` 一次
  push；恢复基线 CI `30325920857 success`。该提交没有部署；远端该时点 HEAD 与生产
  `845f632…` 的运行树等价，但提交 SHA 不同。
- UI-06 结果边界：只用三个紧凑合同 `NarrativeFrame`、`NarrativeBlock`、
  `ReviewerObservation` 和最多一次完整叙事块修复，完成 G3/G4/G7；同一 DeepSeek 的
  Writer 与独立 Reviewer 分请求，服务端不采信 Writer 自报事实标签，并从最终可见成品完成
  闭世界对账。不得新增数据库表、迁移、第二模型／供应商、关键词黑名单、第二修复轮或新平台。
- 当前实现候选：`NarrativeFrame v1` 已冻结四类叙事模式、用户原文来源及允许品牌／商品
  来源；服务端插入不可改写的 `actuality_source`，Writer 只能输出当前模式允许的类型化块。
  独立 Reviewer 不接收 Writer 事实自报，逐 block／scene 返回精确跨度和人物、关系、动作、
  对白、动机、结果、地点、持有物、现实绑定与资源观察，服务端负责完整覆盖、闭世界和模式
  对账。协议缺失直接失败关闭；内容问题最多进行一次、每个 scene 最多关联三个 block 的完整
  块级修复，再完整复审。审后摘要由应用核心在原子保存前复核。
- 当前定向证据：UI-06 合同／适配器、正式 API＋本地 PostgreSQL、UI-05/M7-2A/P1 受影响
  回归合并全绿；正式 API 的 G1—G7 生命周期证明普通交流和不可替代事实问题零写入，G2—G5
  直接 V1，G4 快照冻结原句，G7 Frame／原句不变且 V1/V2/V1 可读，失败运行终态失败且版本
  增量 0。五项 mutation proof 与前端“V1 出现后立即输入”竞态反证通过。以上仍是确定性
  开发证据，不冒充真实 DeepSeek 或生产人工正文审阅。
- 唯一完整本地门已通过：`git diff --check`、ruff、mypy、Golden `155 passed`（含全部
  pytest 与 OpenAPI）、前端 lint、typecheck、interaction test、build 全绿。
- 两份有界审查均允许进入唯一功能候选：产品与内容审查确认用户仍只给种子、系统承担主题／
  观点／结构，G3/G4/G7 不再共享 Writer 自报事实或固定生活场景；工程与安全审查确认快照、
  作用域/RLS 回归、Reviewer 协议失败关闭、一次修复、审后摘要、失败原子性、历史版本与
  前端竞态。审查发现的显式演绎轴、scene 语义对账及 Reviewer 协议误修三项已修复并只复核
  原阻断。真实模型语义仍必须由同 SHA 生产验收证明。
- 前置：UI-04 保持 `CLOSED`。UI-05 Gate A、Gate B、Gate C 及首页动效交接、品牌管理员
  账户安全入口、三登录入口验证码语义的既有完成证据全部保留；UI-06 不重新执行 Gate A/B/C，
  不重做上述冻结能力。
- 本轮原始起点为 `505b90e96a7ea77c57308f4726c91753b07d9016`；强制停止后的恢复起点
  为 `c41f1afea1eb92cb27a3fbfcbc8d37b31d24adbe`。`505b90e..c41f1af` 的 25 个逐次
  push 前向提交及期间连续部署证据完整保留为失控过程证据，本地恢复锚点
  `refs/archive/ui05-responsibility-pre-convergence` 指向 `c41f1af` 且未推送。
- 唯一执行端：当前 WSL 执行端；同一时间只允许一个写入者。
- 结构收敛：候选 A `8d591fac6b4a8d2943ff1ed646f1f476a9a0296f` 删除十个
  stabilize／bind／settle／cascade 辅助层、`SequenceMatcher` 近似商品事实替换和裸意图
  子串目录；`deepseek.py` 从 3,079 行收至 2,374 行，相关适配器测试从 2,642 行收至
  1,966 行。正式闭环收为完整候选 → 闭世界检查 → 完整语义审查 → 最多一次精确单元修复
  → 完整复审 → 原子保存。候选 B
  `f0261e74421c5818afc337aefc42a5b2350dfdd9` 只增加“事实来源不等于拍摄资源”的共享
  约束，没有新增 helper、词表、正则、修复层或迁移。
- 本地证据：候选 A 完整门为后端 `193 passed`，候选 B 受影响门为 `66 passed`、最终
  Golden `194 passed`；`git diff --check`、ruff、mypy、OpenAPI、前端
  lint/typecheck/interaction/build 均通过。两项最小 mutation proof 均能使对应测试失败：
  恢复“让用户选择观点”与恢复“最终审查后静默改写”都不能假绿。
- CI 与部署：恢复阶段承重 CI 共两次，不能声称整个回合唯一。候选 A run
  `30321076710 success`，但生产 G4 否决；候选 B 最终 run
  `30322971553 success`（quality job `90162482680`）。候选 B 镜像
  `sha256:f93dfee63d7552e73418b5fc1338cf8404fbeb20a195ed6746672162dc3fd1e9`
  曾按正式路径部署，schema `20260801_28 (head)`，备份
  `/var/backups/diyu-m5-4/20260728T022552Z-predeploy` 通过 checksum、隔离数据库恢复、
  RLS、应用 readiness 与对象恢复。
- 真实 DeepSeek 最终轮：G1 `PASS`（聊天且 0/0/0）；G2 `PASS`；G3 自动流程完成但人工
  `FAIL`（新增用户未提供的“婆婆带孩子”等孩子／生活情境）；G4 `FAIL`（初审
  `invented_actuality`，一次单元修复后仍为同类失败，版本 0）；G5 `PASS`；G6 `PASS`
  （只问“具体是哪一个月”，0/0/0）；G7 因 G4 无 V1 而不能执行 V1→V2→V1。失败反证
  `PASS`：一次修复后失败关闭，任务/运行 `+1/+1`、版本 `+0`、永久 running `0`。
- 两份有界审查：工程与安全审查确认来源快照、RLS、一次修复、完整复审、失败原子性和前端
  竞态的确定性门成立；产品与内容审查因 G3 人工正文和 G4 真实失败给出阻断结论。真实
  DeepSeek 证据与 deterministic stub／本地测试证据严格分开。
- UI-05 取代结论：用户已确认 successor `UI-06：自然创作与叙事真实性闭环`，只承接
  G3/G4/G7。根因不是继续调整 Prompt，而是 writer 同时自由创作、自报事实性质并自审；
  现有 `non_event / hypothetical / user_presented_actual` 不能稳定区分一般观察、假设推演、
  创作性演绎和真人事实。UI-05 状态为 `SUPERSEDED`，successor 为当前 `ACTIVE` 的 UI-06；
  这不是关闭或通过。
- 生产处置：停止线命中后没有候选 C；数据库未 downgrade，生产已用既有
  `rollback.sh` 回到最后一份有文档证明健康的
  `845f63291ba5060e60f87d1afa5cfc1cdb057e3b`，运行镜像
  `sha256:19ca26328d5402dde6e932e4f7d2d0d096507a6c26199dedfffb2e4253b21e02`，
  公网与回环 readiness 均为 `ready`。恢复开始时只读发现实际生产曾运行第 24 个失控提交
  `1c57d56…`，它不是合格候选；`845f632…` 才是最后一份既有文档证明健康版本。
- 临时材料：两轮合成验收的 10 个任务、10 次运行、6 个版本、活动事件和 2 个已撤销短会话
  已按精确 ID 清理，复核均为 0，永久 running 为 0；生产凭据没有输出、复制或提交。
- 资产边界：本轮激活增量仍为 `0`；运行激活保持 41 条（20 知识、21 方法），候选保持
  243 条，目录保持 25/119（21 项默认可见、4 项体型主动启用），既有缺口状态不变。
- 当前任务包：`docs/UI-06-自然创作与叙事真实性闭环执行包.md`
- 未证明边界保持：不证明任意自然语言都无需澄清，也不证明真实员工采用、真实品牌资料完整、
  真实发布、平台流量、排名、爆款、GMV、销售、企业采用、真实跨租户市场差异、企业 SLA
  或 `20/55/44` 全组合稳定支持；也未证明 G3、G4、G7。
- 唯一下一动作：**提交并单次推送 UI-06 唯一功能候选**；其绿色承重 CI 后按同 SHA 完成
  备份、隔离恢复、部署和不可拼接 G1—G7/H1/D1 真实 DeepSeek 人工验收。持续到
  `UI-06 REVIEW` 或命中已冻结结构停止线。
