...
- 状态：`REVIEW`。2026-07-27（UTC）的主控关闭记录继续作为历史；关闭后用户在生产页面发现
  三个直接验收缺口，已用普通前向提交完成极小修复、唯一 CI、同 SHA 部署和定向生产复验，
  不改写或删除提交历史。
- 前置：UI-04 保持 `CLOSED`。UI-05 Gate A、Gate B、Gate C 的既有完成证据全部保留；本轮
  不是重新执行 Gate C，只修复首页动效交接、品牌管理员账户安全入口和三登录入口验证码语义。
...
- 已关闭的三个直接缺口：
  1. 首页三个临时节奏点已改为竖向关系；临时 SVG 在 `5.2—5.6s` 统一退场，正式
     `/assets/diyu-logo-primary.svg` 自 `5.65s` 淡入，`7.211s` 完成且无临时图形残留；
...
- 当前证据：受影响认证集成测试 `6 passed`；最终 Golden `173 passed`，前端
  lint/typecheck/test/build 全部通过；两份有界审查均 `PASS`；真实 Chrome 在 `6.000s`
  测得临时图形不可见、正式 Logo 可见，生产动效在 `7.210s` 完成，跳过、减少动效、桌面与
  移动均成立。唯一 CI
...
- 未证明边界保持：不证明任意自然语言都无需澄清，也不证明真实员工采用、真实品牌资料完整、
  ...
  或 `20/55/44` 全组合稳定支持。
- 唯一下一动作：主控对首页动画交接、品牌管理员账户安全和三入口第二因素语义做一次定向终审；
  不启动新里程碑。
