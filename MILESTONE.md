...
- 状态：`REVIEW`（2026-07-26 主控终审极小返工已完成，等待重新终审）
- 前置：M7-3、UI-01R、UI-02 已 `CLOSED`；UI-01 保持 `SUPERSEDED`。UI-01、UI-01R 与
  UI-02 的历史原型完整保留，不改写为正式 React 复制模板。
- Git 基线：启动时本地与 `origin/main` 均为
  `5285f7d13cc498eee8c0efff6b5425644119c84b`，工作树干净。
- 初始承重实现：`f27e996e274ce8d360a6876e501d8fa6c01a0e57`；主控终审极小返工最终实现：
  `05fa32ec1bbcbe3f4e2d972234f226df5eb83279`；本轮唯一远程 CI
  `30236466903` 为 `success`。最终状态文档提交见本里程碑收口提交。
- 唯一执行端：当前 WSL 执行端；同一时间只允许一个写入者。
- 已交付：正式 React 公共首页与 A 动效、三套隔离产品空间的入口壳，以及统一创作端正式内容
  主链；复用服务端可信会话、五轴目录、内容任务、自然修改、历史版本、复制导出、素材选择和
  私人默认，没有建设第二套认证、权限或内容业务。
- 产品边界：总部、区域、代理商与门店共用 `/content`，差异只来自服务端可信组织、发布账号、
  表达身份和资格；`/tenant-admin` 与 `/ops` 本轮只完成产品化隔离入口壳，完整业务由 UI-04
  承接。公开首页、三登录入口与错误恢复保持彼此分离。
- 能力边界：目录继续由服务端版本化接口提供，默认可见 21 项，4 项体型相关方向仅在本人主动
  启用后出现；实验、未支持、明确不做和三个来源缺口不进入普通选择面板。UI-03 不批量激活
  资产，不补齐 39 项缺口。
- 生产边界：本里程碑没有部署、没有切换生产默认 UI，没有修改 ECS、生产数据库、Nginx、DNS、
  生产配置或生产内容，也没有创建生产账号或凭据。
- 验证结论：生产模式可信会话＋正式 API＋本地 PostgreSQL 的 V1→V2→V1 链成立；Chrome 150
  在 `1440×900` 与 `390×844` 下完成首页动效、跳过、减少动效、移动“对话/成品”切换和
  长内容检查，动效约 `7.2s`，横向溢出、低于 44px 的可见触控目标、控制台错误和外部请求
  均为 `0`。产品/UI 与工程/安全两份有界审查无阻断；本地完整门通过，Golden/Test 各
  `162 passed`。
- 资产结论：激活增量 `0`，运行激活保持 41 条（20 知识、21 方法）；243/119 投影通过机械
  对账。24 项精简能力进入当前 React 内容消费者，连续系列完整旅程仍由 UI-04 承接。
- 主控返工结果：管理壳与运维壳已删除“已可使用、保持就绪、运行正常”等无服务端依据的状态
  结论，只保留中性任务说明；交互测试同时反证两个产品壳仍隔离且用户可见页面不出现这些结论。
  正式 `/content` 在未给 `target` 时，从当前正式会话允许的内容目标中解析稳定合法默认值，
  并把同一结果用于 `content_scope`、`targets` 与 `bootstrap.current_target`；仅有小红书、
  仅有视频号和原抖音资格均可无参数进入，显式未授权目标继续以人话页面失败关闭，零合法目标
  不创建任务。
- 唯一下一动作：主控重新终审 UI-03；不得部署、不得启动 UI-04。
