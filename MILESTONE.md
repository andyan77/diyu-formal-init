...
- 当前剩余验收：只关闭四项——S2 在原任务形成动作与条件判断一致的新当前版；S3 在原任务
  形成仍属门店人物和当前系列的新当前版；以 H3 V6 为唯一源重建总部小红书、视频号平台任务
  并让三平台父版本可见可核；正式及 maintenance Nginx 的四个 `/activate/` 入口补齐
  error log 丢弃、`Referrer-Policy: no-referrer` 与不携带原始 URI 的 HTTP 跳转。稳定后只运行
  一次完整门、一次远程 CI、一次备份和同 SHA 生产定向复验。
- 唯一下一动作：沿正式 API 与真实 DeepSeek 完成上述四项并重新进入 `REVIEW`。不得启动 M7-3。
