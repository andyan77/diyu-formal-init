...
- Git 基线：本地与 `origin/main` 均为
  `872c657a54e37abf90fb709b512c9b17d404e821`；启动时工作树干净。
...
- 当前边界：不依赖真实资料的连续系列、明确平台载体、真实商品最小事实入口和自然用户
  界面薄连接已经完成，并通过本地完整工程门；现在只推送一个候选、读取唯一 CI 并有界
  部署，随后只向用户提出一次合并的真实资料动作。六篇源成品和
...
- 唯一下一动作：推送唯一工程候选、读取唯一 CI，并在新鲜有界备份后部署同一 SHA；
  生产只读复核无误后进入本里程碑唯一一次真实资料节点。不得启动 M7-3。
