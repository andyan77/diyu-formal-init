...
- 当前任务：在 UI-03 正式 React 产品骨架上接入纯文字 DM01、连续系列、素材管理、完整租户
  管理、笛语运维和公共状态页，并在唯一 CI 后完成有界备份、生产替换、验证与安全回退锚定。
...
- 能力边界：目录继续由服务端版本化接口提供，默认可见 21 项，4 项体型相关方向仅在本人主动
  ...
  资产，不补齐 39 项缺口。
- 生产边界：本里程碑明确包含唯一最终 CI、部署前有界备份、同一最终 SHA 的生产部署、必要
  冒烟和回退能力核验；不修改生产内容语义，不做破坏性迁移，不触碰历史 DIFY。
...
- 剩余验收：完成全业务 React 接入、UI-G03/UI-G04/UI-G07/OPS-G01 与系列/素材接缝、极小
  Chromium 冒烟、一次完整本地门、唯一远程 CI、生产备份部署与冒烟、两份有界审查和权威
  文档收口。
- 唯一下一动作：唯一 WSL 执行端连续完成 UI-04 至 `REVIEW`；不得自行 `CLOSED`，不得启动
  首期以后事项。
