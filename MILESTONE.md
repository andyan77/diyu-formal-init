...
- 状态：`REVIEW`（2026-07-26 候选、资产投影与有界浏览器验证完成）
- 前置：M7-3 已 `CLOSED`；UI-01 已 `SUPERSEDED`，后继为 UI-01R。旧 v0.1 完整保留为
  “页面覆盖盘点与历史反例”，不静默改写为正式设计基线。
- Git 基线：启动时本地与 `origin/main` 均为
  `9dca6e36ca5fd6932de00f16950bd6b799677de7`，工作树干净。
- 唯一执行端：当前 WSL 执行端；同一时间只允许一个写入者。
- 当前交付：现有资产与能力边界总账、能力缺口与补全清单、三套可播放首页叙事动效、五个
  产品空间与四条任务旅程、六张代表性产品页面，以及产品页面之外的独立评审壳。
- 写入边界：只修改文档、只读投影和离线静态原型；不修改正式 React、服务端、API、
  OpenAPI、数据库、认证、权限、生产配置、ECS 或内容数据，不部署、不触发 CI。
- 真源边界：资产 CSV/Markdown 只投影现有 JSON/JSONL 真源，不形成第二套可编辑资产真源；
  候选登记、运行激活、本次调用、已验证能力和真实租户采用继续分开表述。
- 验收结果：243/119 两份投影的来源 SHA、行数、唯一 ID、类型/状态、41 条运行激活、25 条
  精简目录与 6 个直接资产关联通过机械核对；39 项缺口、6 项实验和 1 项明确不做逐项有稳定
  去向。Chrome 以 `1440×900` 与 `390×844` 检查六张代表页、三套动效和独立评审壳，无横向
  溢出、无可见触控目标小于 `44px`，也无控制台错误或外部网络请求；三案约
  `7.07s / 7.25s / 6.78s` 完成，A 案黑点实际移动约 `390px` 并与路径描边同步，减少动效
  直接显示正式 Logo。自然输入到 V1、自然修改到 V2/V3 与历史版本、商品/现场输入到陈列
  V1、局部调整到 V2、需求分类回告及四类进入流程均有可见落点。
- UI 后继路线：`UI-02` 全链路高保真 HTML 原型 → `UI-03` 正式 React 产品骨架与内容主链
  → `UI-04` 全业务接入与生产替换。只有 UI-01R 经创始人确认后才可启动 UI-02。
- 唯一下一动作：创始人选择动效与核心产品方向；不得自行 `CLOSED`，不得启动 UI-02。
