...
- 状态：`ACTIVE`（主控终审返工：只收口五份成品一致性、统一用户可见投影和一次性链接安全）
...
- 当前交付：两个实质不同的演示表达身份及五段画像 V1；两个连续系列各三篇；H1 的
  V1/V2/当前 V3、H3 的 V1—V4、S1 与 S2 的 V1/当前 V2 均可读；H2/S2 冻结一篇前情、
  H3/S3 冻结两篇；
  S2/H2/H3 分别通过 P1/P2/P5；一次透明“幽默玩梗→克制冷幽默”成立；总部与门店各有抖音、
  小红书、微信视频号当前版，共六份，均由 `deepseek-v4-flash` 生成或重编译。
...
- 点名审查：旧 H3 V1 的 P5 错误绿灯已撤销；当前 V4 以两件完整商品的同距离、白色靠前、
  亮黄靠前三段关系，让受众看见“两个人不必同色，也能通过主次保留各自”的穿衣可能，移除
  视觉关系后主要价值消失，故通过 P5；它不证明真人、合身或上身效果。六份平台当前版在标题、
  开头、结构和媒体组织上实质不同；S2 当前 V2 无桌子、桌面、纸、纸笔、笔或马克笔。审查
  A/B 各一次 `PASS`。
- 工程证据：`git diff --check`、Ruff、mypy、Golden、后端 `150 passed`、前端
  lint/typecheck/build/interaction test 全绿；唯一 CI 为
  `9fbac56ab916ab5fd4aaaaa3c2732cf506d921f9` 的 run `30184924584`，结果 `success`，
  后续提交均 `[skip ci]`。
- 生产证据：schema `20260729_23`；已验收的承重代码版本为
  `418442081c26096ddb4fc5adbe004e74f3de9c9a`，其镜像摘要
  `sha256:f7082fec4d35626671f0e63310369808028c752912253d69a5bd7ec0a2afc15e`；
  最终运行镜像以本 `REVIEW` 状态同步提交的生产仓库 `HEAD` 为标签，除状态与证据文档外与该
  承重版本相同；
  新鲜备份 `20260726T045107Z-predeploy`（schema `20260729_23`、成功版本 `242`、完整链
  `214`、对象 `1`）校验通过；回环与公网 readiness `200`，匿名演示索引 API `401`，当前
  容器错误数 `0`。
  前一已知可用镜像 `6f5ee18c066be4f6348660198aaad1f54fa7303e` 及其前一
  `241354127b6cf7213bfb72a094036f0efbc041f1` 保留，回退不做 schema downgrade；历史 DIFY
  未改动。
- 生产数据反证：演示根账号/画像/商品为 `2/2/3`；两个系列各三篇，前情计数均为
  `0/1/2`；六篇源任务与六份平台当前版模型均为 `deepseek-v4-flash`；14 个失败运行均无版本；
  内容/画像/商品/系列/私人偏好关键表 FORCE RLS 为 `true/true`；DM01 仍为任务/运行/版本
  `1/1/1`。
- 未证明：真实员工使用、真实商品采用、真实门店经营事实、真实发布、平台流量/排名、爆款、
  GMV/销售、企业采用、多个真实租户之间的市场差异化、企业 SLA 与 20/55/44 全组合稳定支持。
- 当前剩余验收：沿正式任务和真实 DeepSeek 形成 H2、S2、S3、H3 与总部小红书的新当前版并
  保留旧版；统一页面、复制、导出和历史版本的人话投影；关闭激活 token 日志与并存链接缺口；
  完成本地门、唯一最终 CI、备份、部署和生产反证。
- 唯一下一动作：当前 WSL 执行端连续完成上述有界返工并重新进入 `REVIEW`。不得启动 M7-3。
