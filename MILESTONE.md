...
- 当前交付：两个实质不同的演示表达身份及五段画像 V1；两个连续系列各三篇；H1 的
  V1/V2/当前 V3、S1 与 S2 的 V1/当前 V2 均可读；H2/S2 冻结一篇前情、H3/S3 冻结两篇；
...
- 点名审查：H3 通过的是依赖三段连续画面的可见配色组织可能，不证明真人实际穿着效果；六份
  平台当前版在标题、开头、结构和媒体组织上实质不同；S2 当前 V2 无桌子、桌面、纸、纸笔、
  笔或马克笔。审查 A/B 各一次 `PASS`。
...
- 生产证据：schema `20260729_23`；已验收的承重代码版本为
  ...
  承重版本相同；
  新鲜备份 `20260726T041310Z-predeploy`（schema `20260729_23`、成功版本 `233`、对象 `1`）
  校验通过；回环与公网 readiness `200`，匿名演示索引 API `401`，当前容器错误数 `0`。
...
- 生产数据反证：演示根账号/画像/商品为 `2/2/3`；两个系列各三篇，前情计数均为
  `0/1/2`；六篇源任务与六份平台当前版模型均为 `deepseek-v4-flash`；13 个失败运行均无版本；
