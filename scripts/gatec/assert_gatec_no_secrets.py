#!/usr/bin/env python3
"""Assert Gate C additions contain no credentials or private evidence paths."""
...
from assert_gatec_scope import EXPECTED_BASE_SHA, changed_paths
...
_SELF_PATH = "scripts/gatec/assert_gatec_no_secrets.py"
...
def main() -> None:
    ...
    untracked_text = "\n".join(
        ...
        for path in sorted(untracked & changed_paths(root))
        if path != _SELF_PATH and (root / path).is_file() and not (root / path).is_symlink()
    )
    scanner_source = "\n".join(
    ...
    if hits:
        raise SystemExit(f"Gate C privacy FAIL: {hits}")
    ...
    if frozen_diff:
        raise SystemExit(f"Gate C privacy FAIL: frozen paths changed: {frozen_diff}")
    print("GATEC_NO_SECRETS_OK secret_hits=0 private_path_hits=0 frozen_path_diffs=0")
