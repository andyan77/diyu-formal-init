...
- 状态：`DRAFT-FOR-REVIEW · REVISION-3`（本文件为完整修订草案，待守护审查；根目录
...
- 配套文件：[UX-04R 前端产品化增量与工程边界执行包（完整修订版）](UX-04R-前端产品化增量与工程边界执行包-完整修订版.md)。
...
### A7 · 有界创作参谋 `BoundedAdvisorV1`（中—重：新运行合同）

不另建通用 Agent 平台，但必须建立真实的参谋能力，而非只有机会卡和生成确认。
...
| 工作项 | 风险级 | 流程 |
|---|---|---|
| A2 / A4 / A5 / F2 / F3 | 轻 | 侦察通过后合并执行，delta-only 验证 |
| A1 / A3 / A6 / A7 / B2 / B5 / F1 / F5 | 中 | 单项 Brief + 守护审查，可合并相邻步骤 |
