...
- 修订记录：REVISION-5（2026-08-06）落实 Codex 审查 `PASS_WITH_BOUNDED_CORRECTIONS`
  六项有界修正（D-COMM-08 定稿、A7.5.3 basis_ref 例外、模型知识主张边界、B5 第 10 指标、
...
### D-COMM-08 · GKB 行业知识库不接入 COMM-01 主线（Codex 审查定稿版）

founder 2026-08-06 裁决（经守护审查 `PASS_WITH_BOUNDED_CORRECTIONS` 六项有界修正后定稿）：

1. COMM-01 主线——包括 A7 交互编排、意图识别、内容创意参谋、`CreationProposalV1` 与
   `ProductionPackageV1` 全部路径——**不得在运行时读取、检索、向量召回或依赖** GKB
   行业领域通用知识库。
2. 行业通用、低风险、非权威的创作建议由**模型参数知识**生成，受**版本化 Advisor Prompt
   合同**约束，并由**五轴表达目录**提供合法表达控制选项。三者职责不得混同：模型参数
   知识提供通用建议；Prompt 合同约束行为与诚实边界；五轴目录提供稳定可选控制项，
   不构成行业事实来源。
3. 此类建议统一标记 `knowledge_origin=model_parametric`，用户文案为**"行业通用参考
   （模型知识，需人工判断）"**。它**不得生成 `basis_ref`**，不进入 `BrandBasisItemV1`、
   `context_selected` 品牌依据 items、`CreationProposalV1.brand_basis_refs`、成品
   `BrandBasisPanel`、`brand_basis_feedbacks` 或任何品牌依据计数（含北极星）。与品牌
   依据共同形成建议时，两类理由必须分开呈现。可作为 Proposal 的
   `system_recommendation` / `selected_angle` / `content_form` / `accepted_assumptions`
   的形成参考，但必须冻结来源类型。
4. **模型参数知识的主张边界**：不得取得品牌事实、商品属性与功效、价格、库存、真实
   消费者/门店/机构经历、当前平台政策、当前平台算法与流量规律、经营效果承诺、法律
   合规结论、健康/身体/性能类主张或其他需权威时效出处的事实资格。缺少已确认来源时
   必须标记为建议、未知或不可用；hard / performance 类主张不得引用模型知识或专家合成
   来源。平台与形式建议可引用既有版本化平台方向合同（`platform_directions`），但不得
   以模型知识声称当前平台规则。
5. **快照冻结口径**：模型知识不具备条目级来源追溯；系统只冻结当时呈现并被用户确认的
   建议文本、`knowledge_origin`、模型部署标识、Advisor Prompt 合同版本/digest 与五轴
   目录版本，**不承诺**未来重新询问会生成同一句建议。
6. GKB 项目**暂停继续扩建**。已有资产保持来源与历史状态，不得被 A7 隐式加载、不计入
   COMM-01 能力或就绪度。保留三个**无运行时依赖的低成本复用途径**（防隐式复活边界）：
   - 经一次性人工筛选、编辑与回归后，蒸馏为**短小、版本化**的 Advisor Prompt 合同素材
     ——不得把原始 GKB 大段塞入 Prompt、不得运行时查询、不得显示为可引用来源；
     Prompt 合同变化必须触发 A7 回归验收；
   - 保留为 DM01 陈列域候选：不自动进入 DM01 运行规则、不自动取得证据等级，另立
     Brief 与适用性验收后方可激活；
   - 保留为选题种子候选：仅作可修改启发，不作品牌事实或效果承诺，不自动进入当前
     `opportunities` 输出。
7. **重启门（只触发重新评估，永不自动接入）**：① B5 第 10 指标（参谋质量反馈率）超过
   观察前冻结的阈值且达最低样本量；② 问题在至少两个独立操作者或两个独立使用周期重复；
   ③ 根因分析确认主因不是品牌资料缺失、Prompt 合同缺陷、UI 表达误导、模型版本故障或
   用户未理解建议性质；④ founder 重新裁决；⑤ 另立 GKB 接入 Brief。
8. **不受影响范围**：本裁决不撤销五轴表达目录、既有版本化平台方向合同、确定性媒体
   程序与制作规则、租户品牌发布投影、商品事实管道、账号画像与 DM01 已确认正式资产；
   仅禁止 A7 / COMM-01 在运行时读取、召回或依赖 GKB。
...
- 不接入 GKB 行业知识库，不为行业通用知识建设或扩建独立运行时知识库（D-COMM-08）。
...
**六层参谋依据与约束栈（系统凭什么给建议，founder 2026-08-06 补充裁决）——逐层标注现状。
命名说明：模型参数知识、Prompt 约束、目录控制与品牌事实不是同一种意义上的"知识库"，
故不称"知识基座"：**

| 层 | 内容 | 首期来源与现状 |
|---|---|---|
| 1 行业通用知识 | 自媒体内容运营专家方法论：选题方法、平台特性与节奏、内容形式与结构、表达技巧 | **已裁决不接入 GKB（D-COMM-08）**：行业通用软性建议由模型参数知识生成、受版本化 Advisor Prompt 合同约束、由五轴目录提供合法表达控制项（三者职责不混同）；统一标注"行业通用参考（模型知识，需人工判断）"，不构成可审计事实来源、不进入品牌依据 |
...
**分层引用诚实性（硬边界）**：行业通用方法论建议必须标注"行业通用参考（模型知识，
需人工判断）"；品牌事实必须带来源版本；**不得把行业通用知识冒充品牌确认事实**；
hard / performance 类主张不得引用模型知识或专家合成来源（沿用 GKB source policy 既有红线，
完整主张边界见 D-COMM-08 第 4 条）。

**ChatGPT 体感基准（founder 2026-08-06 裁决，产品体验原则）**：前端交互体感应**接近
ChatGPT**——单一自然语言输入框、流式响应、随时继续追问、结构化卡片作为对话流内消息
渲染而非跳页表单、无强制表单感；**但系统不是另一个 ChatGPT**——能力白名单（A7.3）、
品牌依据与诚实标注（A1 / D-COMM-08）、任务承诺门（A7.6）、有界自由对话口径（选项甲）
全部不因体感优化而放松。体感是壳，有界是骨。
...
- 硬性事实问题仍只限现有不可替代事实边界；
- 推荐理由的来源分类（D-COMM-08 修正）：
  - **品牌、账号、商品或已登记素材特定**的推荐理由，必须绑定一个或多个公开 `basis_ref`，
    但不声称逐句因果；
  - **纯行业通用方法建议**不得生成或伪造 `basis_ref`，必须标记
    `knowledge_origin=model_parametric`，显示为"行业通用参考（模型知识，需人工判断）"；
    不进入 `BrandBasisItemV1`、`context_selected` 品牌依据、
    `CreationProposalV1.brand_basis_refs` 或品牌依据计数；
  - **混合建议**同时使用品牌依据与模型通用知识时，两类理由必须分开呈现：品牌特定部分
    绑定 `basis_ref`，通用方法部分标记 `model_parametric`。
...
| 任意 | 普通选题方法建议 | 显示"行业通用参考（模型知识，需人工判断）"；`basis_ref` 为空，不增加品牌依据数量 |
| 任意 | 品牌 / 商品 / 素材特定推荐 | 必须绑定真实 `basis_ref` |
| 任意 | 混合推荐 | 品牌依据与模型通用建议分项显示，不得合并计数 |
| 任意 | 商品性能 / 当前平台算法问题 | 不得以模型知识作已确认结论；标注建议、未知或不可用 |
| 任意 | Proposal 冻结 | 通用建议进入 `system_recommendation` / `accepted_assumptions`，不进入 `brand_basis_refs` |
| 任意 | 成品依据面板 | 不显示模型通用知识为品牌依据 |

**知识来源工程断言（fail-closed）**：A7 全部路径运行时 GKB 调用次数为 0——无 GKB
adapter / endpoint / vector 检索调用；无 GKB 配置成为 A7 启动前提；GKB 缺失不影响
Advisor 可用；Prompt 蒸馏产物落地后无任何运行时 GKB 查询。参谋质量反馈进入 B5 第 10
指标数据源，**不进入** `brand_basis_feedbacks` 品牌资料反馈队列。
...
10. 参谋质量反馈率（**人工运营指标**，D-COMM-08 重启门唯一数据源）：分母 = 展示过
    至少一条 `knowledge_origin=model_parametric` 建议的独立参谋会话（自动事件计数）；
...
15. 自由对话有界口径（选项甲）的文案、路由与验收是否一致；通用请求既不被静默受理、
    也不被生硬拒绝？
16. D-COMM-08 是否贯穿一致——模型通用建议零 `basis_ref`、零品牌依据计数；"行业通用
    参考（模型知识，需人工判断）"渲染硬验收是否可执行；Prompt 蒸馏是否有防隐式 GKB
    复活边界；B5 第 10 指标分子/分母/冻结规则是否可操作；重启门是否只触发重新评估、
    永不自动接入？
