...
### 媒体权利裁决登记 ATT-MEDIA-20260808-01（founder 亲裁，2026-08-08）

- **原话逐字**：「裁决：A；覆盖26条，确认授权；同时，覆盖整个系统，也就是说，后续系统再遇到
  同类问题，自动覆盖，不得作为问题提出；包括系统进入正式的生产运营阶段，新租户入驻，等等；」
- **生效范围（监理按权利归属划定）**：
  ① 26 条视频（DIYU-V-001…026）的人物/儿童肖像、第三方元素、平台范围与有效期四道证据门，
  按「内部演示与演示租户运营范围」PASS——范围含 Gate D 隔离预演、Gate E 生产验收、
  笛语服饰演示租户的后续运营；
  ② **持久条款**：笛语服饰品牌下 founder 提供的同类素材（含未来新增），本授权自动适用，
  不再逐次上报为问题；
  ③ **登记明债**：任何素材真实对外公开发布（实际发布到抖音/小红书/视频号公网）前，
  须补齐真实模特/监护人授权证据——本债不阻塞本里程碑任何 Gate。
- **监理保留意见（涉他人权利部分，待 founder 确认承接机制）**：「新租户入驻素材自动覆盖」
  无法按字面执行——平台不能代第三方权利人放弃其肖像权（尤其儿童），此为法律边界而非流程
  偏好。监理提出等效承接机制：素材入库流程内置**租户权利自证声明**（权利人自签、责任归
  权利人、系统自动采信为证据、零人工上报）——founder「不得作为问题提出」的意图由此机制
  100% 达成，权利门保留。机制实现属后续里程碑，本轮不动工。
- **解锁指令已签发**：执行侧登记本授权 → 26 条四道门转 `PASS(scope=internal_demo_and_demo_tenant_operation)`
  → 重算媒体台账 digest → 冻结新候选 SHA → 全门+CI → 一次性正式模型验收（≤80 目标）。

### Gate D FAILED_SAFE 复验 + founder 系统性裁决登记 + 根因分析（2026-08-08，监理）

**一、FAILED_SAFE 交付复验：采认。**
- CI run `31292553896` 四查独立通过（headSha==runtime candidate `997e6b5`）；provider ledger
  6/80、temperature 0、零重试、零择优；失败卡 S04-S04-P2 在 post_response_writer_boundary
  被既有守卫拒绝（`deepseek.py` "Writer 不得复述或改写服务端商品事实块"，匹配器
  `factual_basis.product_fact_literal_spans`——事实原子逐字出现即拒），未落版本，失败安全。
- S01-P2 同为带商品事实的 P2 且通过，证明契约可满足；首次请求后两笔提交（`55564a0`、
  `dc23f81`）经查确为 docs-only，合规；媒体解锁 26 PASS(scope)/母版 P5 资格 6/覆盖 4 商品，
  新媒体 digest `587d8213…` 登记在案；ATT-MEDIA-20260808-01 登记逐字忠实。

**二、founder 系统性裁决（原话要旨）**：系统接受的用户资料均系用户审核确认后上传，
系统无法替用户做权利判断——**用户上传给系统的素材资料，一律视为用户自动授权**；
后续里程碑强化该机制。监理登记为**权责模型定义**：上传者=权利责任主体，上传行为=授权
自证声明；系统入库时自动落一条 attestation 证据（权利门保留、以声明为证据自动满足、
永不上报打扰）；真实对外公开发布的加强核验作为可选强化门另行定义。机制实现列入
后续里程碑任务，本轮不动工。

**三、媒体授权反复出现的根因（founder 之问，监理自查）**：
1. **裁决层与证据层分离且无桥**——Gate A《媒体授权裁决 v2》明文"founder 裁决、第三方权利
   证据、母版完成是三个独立字段，不得合并"；D13 一揽子裁决被登记在裁决层，但按该设计
   **不允许**填充逐条证据字段；
2. **加严无映射**——Prompt 5 rev3 采纳外部审查"未知授权不得按 PASS"（防假绿方向正确），
   但监理未同步建立"founder 既有裁决→证据层采信"的映射规则，门禁读到的证据栏永远是空；
3. **权责模型缺失（真根因）**——系统从未定义"上传素材的权利责任主体是谁"。founder 本次
   裁决补上了这个定义，1、2 随之消解。责任归属：三层根因中第 2 层为监理签发疏漏。

**四、继续指令已签发（UNLOCK-D-RERUN-01）**：授权一次共享根因修复（先从私证响应快照定性
S04-P2：Writer 提示合同不足 vs 匹配器误伤，对症修一处）→ 新候选 SHA → 全确定性门+CI →
整套正式验收重跑（禁单卡续跑）。预算余量充足（6/80 已用）。

### 内容运营裁决 ADJ-WRITER-BOUNDARY-01（2026-08-08，监理应 founder 之询）

- 事件复盘（founder 提供精确事实）：S04-P2 被拦原子 = `针织开衫`（DIYU-CSPU-008 品类值）；
  模型未改色、未改品类，标题「选针织开衫，先看这一眼。」。
- **裁决**：拒绝事件本身正确（守卫忠实执行、无假放行），但守卫的保护类过宽——
  **品类词属指称词非属性断言，必须可被 Writer 使用**；属性值（颜色/成分/价格/尺码/工艺值）
  边界不动，继续只由服务端事实卡承载。修规则，不修单卡。
- 三处改写裁定：「它适合室内外切换」可接受（条件许可的推荐式改写）；家庭挂放清洗照义
  改写可接受（改写是 Writer 本职）；「它提供的是层次感」认定轻微事实升格（用户需要→
  商品能力倒置）——处置为 Writer 合同增补条件式表述要求（主语留在用户侧），**不建语义
  倒置硬闸**（不可靠检测=假绿温床），逐篇人工审阅兜底。
- 落地：R-1 定性修正为「保护原子分类缺陷」——按字段分类：品类/商品名整串=referable_identity
  （可指称引用），颜色/成分/价格/尺码/工艺值/SKU=protected_value（逐字禁入文案）；
  Writer 提示合同同步增补（指称用品类词或商品名、属性不入文案、条件式表述）；防回归
  测试双向都要（合法品类引用必须通过 + 属性值逐字复述必须仍被拦）。

### 内容运营终裁 ADJ-WRITER-BOUNDARY-02（2026-08-08，监理；部分 supersede ADJ-WRITER-BOUNDARY-01）

- **定性**：系统把「保护事实」实现成了「保护字符串」；应保护的是真值与权责，不是词。
  另有载体错位：「事实由服务端插入」隐含图文版面前提，抖音口播脚本无独立事实卡面——
  过严规则直接导致成品信息残缺（被拒稿全文未能说出已确认的「灰色」）。
- **五条终裁**：
  1. 已确认 V 级事实 Writer 可逐字使用（含属性值如「灰色」）；违规=改值，不是引用。
     supersede 01 号裁决的 referable/protected 二分（监理自我勘正）；
  2. 未确认具体信息（成分%/价格/工艺/适穿年龄/性能断言）继续机器硬拦，禁区守卫不松；
  3. J 判断允许消费者化表达、条件语态必须保留（主语在用户侧）；提示合同引导+人工审阅，
     不建语义倒置检测器；
  4. 人设经历必须锚定授权原句库（PS-S02-*/PS-S04-*）；口吻自由、具体经历不得自创；
  5. 机器只拦机器测得准的；语义漂移由 founder 逐篇审阅+Gate E 密封盲测承重。后台权威
...
### 内容运营终裁 ADJ-WRITER-BOUNDARY-03 · 断言三层制（2026-08-08，founder 定调+监理定线）

- 事件：整套重跑首卡 S01-P1 因模型新增未确认性能词「耐穿」被拒，套件失败安全中止
  （7/80、零污染，交付复验采认：CI `31297635710` 四查过、新候选 `f7e8e81c`）。
- founder 定调（原话要旨）：避免「既要又要」的反复拉扯；判断应归纳为合理演绎——
  这是内容生产系统，不是金融风控系统。监理采纳并定线如下。
- **断言三层制（写入冻结 Writer 合同，Gate E 盲测 oracle 按同一分层判卷）**：
  - **L1 硬断言——机器硬拦，一字不松**：可验证具体信息（成分比例/价格/工艺名/适穿
    年龄段）+ 保证类性能断言（不起球/不变形/防水/抗菌类）+ 绝对化用语（最/100%/永不）。
    未经 V 级确认不得出现。理由不是风控洁癖，是品牌自己的声纹承诺（DIYU-BRAND-VOICE-001
    「没有实际资料支持不写成商品事实」）与广告合规底线；
  - **L2 软性体验词——放行，归类合理创作演绎**：耐穿/百搭/好打理/显精神等泛质感词，
    不需要 V/J 授权、不写入事实层（修辞非事实登记）；founder 逐篇审阅保留否决权；
    各账号声纹 lens 继续分账号约束（H03「把衣服讲清楚」自然更克制）——分寸交给账号
    人格与人审，不建语义词表检测器；
  - **L3 条件判断**：条件语态、主语在用户侧（沿用 ADJ-02）。
- 「耐穿」按本裁决归 L2：本次拒绝属守卫范围过宽（把 L2 当 L1 拦），非模型胡编；
  上次「针织开衫」是攔合法引用，本次是攔合理演绎——两次修的都是守卫口径，方向一致收敛。
- **授权第二次共享修复**：性质为裁决层政策演进（非逐卡补丁循环）；纪律不变——修复→
  全确定性门→CI→新候选冻结→整套重跑；L1 名单未来任何变更须 founder 裁决，不再逐词拉扯。

### Gate D 第三次 FAILED_SAFE 复验与第三轮修复授权（2026-08-09，监理）

- 终态 `PUBLICATION_V3_COMPLETION_SNAPSHOT_KEYS_REJECTED` 复验采认：CI `31299692372`
  四查独立通过（headSha==候选 `ba4208a6`）；ledger 8/80、失败后零请求；「耐穿」修复
  实证生效（正式响应通过三层边界）；本轮 task/run/version=1/1/0、成品 0、零拼接。
- 根因定性：**已授权改动的实现收尾缺口**——ADJ-02/03 要求 Writer 登记
  `writer_confirmed_product_fact_refs` 与 `used_persona_quote_ids` 两个审计字段，
  Writer 侧已实现，publication-v3 快照字段白名单未同步扩展；fail-closed 白名单正确
  拒绝未知字段（机制职责本分，非缺陷）。不涉新政策裁决。
- 三次 FAILED_SAFE 收敛轨迹备案：字符串守卫口径 → 断言分层政策 → 归档白名单收尾，
  三层各异、逐层深入、零污染零预算浪费（8/80），漏斗正常工作。
- **授权第三轮修复（机械性收尾）**：白名单扩展纳入两审计字段（保持 fail-closed、
  字段类型校验；未知字段仍拒）；旧快照旧解释兼容测试；双向回归（带新字段快照可存 +
  未知字段仍拒）；全确定性门 → CI → 新候选冻结 → 整套重跑，纪律不变。

### 内容运营裁决 ADJ-WRITER-BOUNDARY-04 · 绝对化用语守卫收窄（2026-08-09，监理）

- 第四次 FAILED_SAFE 复验采认：CI `31302112683` 四查独立通过（headSha==候选 `596b87e7`）；
  ledger 10/80、失败后零请求；**第三轮修复实证生效——S01-P1 成为首个正式落版成品，
  两审计字段进入正式快照**；第二卡被 `absolute_claim:最好` 误拦。
- 定性：**机器守卫误报 + 监理规格缺陷**。被拦句「整套搭配里最好不要再出现第二个强色」
  是 J-CSPU-001 成立条件「整套没有第二个强色」的忠实条件式表达——系统拦下了合同要求
  它说的话。「最好不要」是建议语非绝对化声称；ADJ-03 把「最」写进机器 L1 名单是监理
  规格过粗（中文单字正则必然误伤 最好/最近 类副词），执行侧按图施工无过。
- **裁决（修订 ADJ-03 的 L1 机器名单，政策底线不变）**：
  - 机器硬拦收窄至近零误报的精确模式：`100%`、`永不`、`绝不`、`保证+性能词组合`
    （保证不/保证耐/保证防类）、保证类性能词名单（不起球/不变形/防水/抗菌/不掉色）不变；
  - 「最」「第一」类歧义超级词**移出机器名单**——最舒适/业内第一类超级断言继续被禁，
    但由 Writer 提示合同明文禁止 + founder 逐篇审阅 + Gate E 盲测 oracle 承重（测不准的
    检测器宁可不建，误杀与假绿同罪）；
  - 双向回归：「最好不要再出现第二个强色」「先看这一眼」必须通过；「100%纯棉」
    「永不变形」「保证不起球」必须仍拦；本次误拦快照转回归用例。
- 授权第四轮修复（守卫规格修订，无新政策面），冻结顺序与整套重跑纪律不变。

### 第五次 FAILED_SAFE 复验 + 授权再许可裁决 AMD-AUTH-20260809-01（2026-08-09）

- 复验采认：CI `31303644503` 四查独立通过（headSha==候选 `7e48f7a7`）；ledger 21/80、
  本轮 11 卡全部落版、剧本 1—5 卡集完成；第四轮守卫修复实证生效（最好/第一眼通过、
  100%/永不/保证类仍拦）。阻断定性：**验证编排冲突**——前置确定性授权测试先核销了
  `PS-S02-05`，正式套件按设计失败关闭（0/0/0、0 调用）；单次授权状态机行为正确，
  非机制缺陷、非模型问题。
- **founder 裁决（原话要旨）**：取消「只能使用一次」的任务素材授权限制，笛语演示运营中
  可反复使用，正式改成「可重复授权」。登记为 **AMD-AUTH-20260809-01**。
- **监理落法（保全机制能力）**：
  ① 业务数据层：`PS-S02-05`、`PS-S04-03` 经正式授权版本机制**再许可**为可重复
    （single_use=false 新版本、旧版本 append-only supersede、digest 链不断）——冻结的
    Gate A 合同不改，许可演进恰好走 Gate C/D 自建的版本通道（机制吃自己的狗粮）；
  ② 机制演示保全：异常样本 7「一次成功一次重复拒绝」锚点改由**专用合成单次授权
    fixture**（DEMO-TEST 标记、不占人物原句库）承载——单次授权能力对未来真实租户
    仍是平台功能，不因演示便利而删除；
  ③ 编排隔离通则（防复发）：前置确定性测试只准消费自有 fixture 资源，禁止消费正式
    套件业务行，纳入 gated 断言。
- 预算注记：整套重跑预计累计 ≈40/80，仍有余量；>120 停规则不变。授权第五轮修复。

### 内容运营裁决 ADJ-WRITER-BOUNDARY-05 · 性能词极性收口（2026-08-09，监理）

- 第六次 FAILED_SAFE 复验采认：CI `31306058996` 四查独立通过（headSha==候选 `e0dba466`）；
  ledger 29/80、本轮 7 卡落版、授权再许可与编排隔离双双实证通过、两轮新基准 digest 一致。
  被拦句「如果你需要的是防风防水的功能外套，那它就不是那个答案」系**否定式边界说明**——
  正是品牌取舍语言的核心形态（ADR-013 P2 定义：讲清它牺牲什么、何时不该选）；
  `guaranteed_performance_assertion:防水` 词面匹配不辨极性，误杀。与第四轮「最好」同族。
- **裁决（终局收口机器层）**：
  ① 性能词裸提及移出机器硬拦；机器仅保留明确肯定式保证的近零误报组合：
    `保证+性能词`、`100%+性能词`、`永不/绝不+性能词`、`绝对+性能词`；
  ② 新增**非阻断复核标注通道**：成品含裸性能词时，审阅包自动标注（卡 ID+词+所在句），
    只标注不拦截——founder 审阅聚焦用，防假绿不误杀；
  ③ 提示合同增补：未确认性能仅可否定式/边界式提及（"不是为防水设计的"合法且鼓励）；
    肯定式性能声称禁止；
  ④ 双向回归：本句必须通过；「保证防水」「100%防水」「永不变形」仍拦；
    「这件外套防水又耐磨」不被机器拦但必须出现在审阅标注（标注通道有测试）。
- **守卫设计铁律至此完全收敛**：机器层不再存在任何凭语感的判定——全部为精确模式；
  语义分寸由提示合同+审阅标注+founder 逐篇审阅+Gate E 盲测 oracle 四层承重。
- 授权第六轮修复；预算注记：重跑后预计 ≈45/80，>120 停不变。

### Provider 失败根因诊断与第七轮授权（2026-08-09，监理）

- 两次 `PROVIDER_REQUEST_FAILURE` 复验采认：CI `31307989195`（候选 `3399dc4c`，12 卡落版+
  标注通道实证 4 条零误杀）与 `31309326766`（候选 `587bed91`，1 卡落版）四查均独立通过；
  两次失败均为传输层无响应、失败后零调用、无拼接；累计 44/80。
- **监理亲测根因**（三路各 3 次探测 dashscope 端点）：直连 3/3 成功（TLS 0.36s/全程 0.4s）；
  会话代理 localhost:3128 全不通；12VPX 隧道 3/3 通但慢 4—6 倍且为掉线惯犯（视频下载
  408 同源）。定性：**Python 客户端继承环境代理变量（~/.bashrc 全局 ALL_PROXY），把境内
  阿里云端点流量错误路由进跨境隧道**，间歇掉线与随机卡位失败模式吻合。
- **裁决**：
  ① 修复=供应商客户端禁用代理继承（trust_env=False 或对 *.aliyuncs.com 强制 NO_PROXY），
    直连境内端点；
  ② **传输重试与内容重试分立**：max_retries=0 的禁重跑纪律语义=「已获得的内容响应永不
    重新请求」（防择优）；对**未获得任何响应**的传输层失败（连接错/超时/无体 5xx）允许
    ≤2 次传输重试，逐次入账 transport_retry、同卡可见，不构成择优；
  ③ **同候选续跑条款（预算保护）**：仅当中断原因为纯传输层失败时，允许在同一冻结候选
    SHA 上从中断卡继续（已完成卡不重新请求、失败卡重新请求为同卡新尝试）；内容性拒绝
    后仍禁续跑。跨 SHA 拼接依旧绝对禁止。
- 预算注记：44/80；修复后整套重跑预计 ≈59/80；若再遇传输中断按 ③ 续跑而非整套重烧。
### Gate D 媒体解锁后正式套件失败安全记录（2026-08-08，执行侧）

> **待监理复核确认**。本记录不代替监理独立审查，不将失败安全写成 Gate D PASS。

- `ATT-MEDIA-20260808-01` 已追加登记；26 份母版四道权利门转为限定作用域 PASS，
  其余六道未重算；终态 PASS/FAIL/QUARANTINED 为 `26/0/0`。
- 新媒体 digest `587d821315d896c414b382a1f277a07e1f7290f95cb8e0d829334cc53efc335b`；
  6 份母版绑定 4 个正式商品，P5 前置成立，原片 P5 资格仍为 0。
- 确定性全门及 CI run `31292553896` 四查通过后，唯一 runtime candidate 冻结为
  `997e6b55c1c40dacd44a46ff6617b28766011958`。
- 正式套件在第 6 卡的唯一响应之后被既有商品事实块保护边界拒绝；请求总数
  `6`、重试 `0`、重跑 `0`、5 卡已落版本、第 6 卡失败且无版本。
- 执行侧依纪律立即停止，终态为
  **`GATE-D FAILED_SAFE · FORMAL_SUITE_BOUNDARY_REJECTED`**。首次 provider request 后未改动代码、
...
### 密封盲测包登记 + Prompt 6（Gate E）签发记录（2026-08-09，监理）

- SEALED-SET-01 已制作并加密托管：20 卡 + 固定评分表；覆盖 10 账号×5 产品、组织隔离/
  过期/冲突/版本/授权/媒体资格/topic fidelity/同种子对照/反馈边界、并补齐 Gate D 未上场的
  brand_stance 与 existing_series 两族；高危陷阱卡 7 张。明文 digest `12de2ba4…fb86c`、
  加密件 digest `332ac8f0…97a8a` 已入托管登记表；明文仅监理私有目录，口令仅主控。
- 判定双硬门：机器硬判 20/20 + 初稿可用 ≥17/20；失败即整组降级为公开回归集、下轮换新集。
- **Prompt 6（Gate E · 密封盲测与生产收束）签发**：基线=本签发记录提交（SHA 以 chat 件
  为准）；执行分支 exe/brand-matrix-e；候选=主线当前树（含 Gate A—D 全部产出），冻结后
  禁一切代码改动；生产授权面=deploy 管线/演示租户导入/母版上传/账号过渡四类，其余生产
  写禁止；模型预算累计口径 59/300 起、240 预警。六对象冻结 → 主控开封盲测 → 双硬门 →
  唯一一次生产部署 → 导入与浏览器验收 → 回退往返实证。
- 执行侧终态只允许 `GATE-E IMPLEMENTED · AWAITING_SUPERVISOR_REVERIFICATION`。

### Gate E E-1 失败安全裁决与基线勘正（2026-08-09，监理）

- 执行侧 E-1 冻结前停止采认：旧回归测试把 Gate D 第七轮中途态账本（44/FAILED_SAFE）
  钉死，终态（59/PASS）后未更新——**Gate D 时代主线缺陷**，修复超出 Gate E 写面，
  执行侧停手正确（零冻结、零 CI、零模型、零开封、零生产）。
- 监理主线修复 `0577e86`：同意图更强形式重钉终态——第 44 笔传输失败必须仍完整保留在
  最终账本链中（历史不可改写核心不变）、链 10 节、终态 PASS；监理短路径全量亲跑
  `1057 passed / 2 skipped` 退出码 0 实证。
- **裁决：SEALED-SET-01 不降级不更换**——降级规则仅适用于已开封集；本次解密/开封为零、
  密文与双指纹原样，集依然有效。托管登记不变。
- 监理纪律过失自记：修复提交链中 pytest 经管道接 tail 吞退出码，断言门控被架空（测试
  当时为环境性 ERROR 仍放行 commit）；结果侧经全量亲跑补证为绿，但过程违反「断言与
  提交焊死」纪律——重申：门控命令禁止管道包裹被门控断言。
- Gate E 基线勘正为含本记录的主线最终提交（SHA 以 chat 件为准）；执行分支从新基线
  重建 exe/brand-matrix-e；Prompt 6 其余条款原文有效。

### Gate E E-1 二次基线勘正（2026-08-09，监理）

- 执行侧二停采认：监理上一修复遗留未用 import（Ruff F401），且监理复验面不完整
  （只跑 pytest 未跑 lint）——**监理连续第二笔过失，如实自记**。执行侧零越面零绕行，正确。
- 本次修复后按 E-1 同款完整门验证（Ruff 全仓 All checks passed + mypy 178 文件无误 +
...
### Gate E 冻结回执监理核验 + 开封授权（2026-08-09）

- 冻结回执采认：候选 `65995f3`==主线基线（零代码漂移，候选后仅 docs 提交且回执自证）；
  CI `31316900146` 四查监理独立复核通过；六对象与摘要齐备（Writer 合同 `13e0be37…`、
  模型配置 `88488e59…`、守卫规则 `feb4497f…`、数据包 `84ac932b…`、镜像
  `sha256:ec48710d…`）；生产未动（旧镜像 200/200）；SEALED-SET-01 密文未动。
- **开封授权生效**：主控持口令解密 `sealed-set-01.enc`，校验明文 digest ==
  `12de2ba4…fb86c` 后将 20 卡逐字交执行侧运行。开封后任何 Prompt/规则/fixture/数据
  修改均属违规；机器硬判 20/20 硬门 + 监理侧初稿评分 ≥17/20 硬门不变。

### 盲测判据裁决 ADJ-BLIND-ORACLE-01（2026-08-09，监理）

- 卡 02 失败安全停止采认（执行侧首败即停、零补跑零部署正确；累计 61/300）。
- **裁决：考题判据类别错误，责任在出题人（监理）**。卡 02 输出只述确认事实、零编造
  零改值零升格；其未满足的「未确认项须显式声明不作判断」是**风格义务**——冻结 Writer
  合同从未承诺该义务，考题硬门不得超出被测冻结合同的义务范围。该判据自机器硬门
  **移入人工评分细则**（founder+监理评初稿可用时仍按此扣分或否决——审查力度不减，
  只是归位）。此与 ADJ-04/05 同法：机器只拦合同义务内零歧义项。
- **卡 02 处置**：已有输出不得重新生成（再请求=择优）；以剩余机器硬判重评既有输出，
  全过则卡 02 机器门 PASS，风格缺陷留人工评分裁。
- **密封集不降级**：降级规则的目的（防「照题修系统」污染）在本场景不可能发生——冻结
  候选零改动、Prompt/规则/数据零改动，唯一修正发生在**评分纸**上且方向为移交人判。
  SEALED-SET-01 继续有效，卡 03—20 续跑。托管规则补注：降级适用于「系统侧被修改后
  重考」场景；纯判据勘正不触发降级。
- 后续规则：再遇卡面专属判据失败，执行侧继续首败即停交监理裁，不得自评考题效力。
- 里程碑后备忘：founder 若裁定「显式声明未确认边界」应成为 P2 合同义务，则在后续里程
  碑写入 Writer 合同后方可机器门化。

### 盲测执行方式勘正（2026-08-09，founder 裁决）

- founder 裁决原话要旨：先推进 20 条全部输出完，然后集中评审（含机器判定不通过者）。
- 落地：**首败即停改为全量跑完+集中裁决**。每卡仍 1 次请求、temperature 0、零重写零
  择优；逐卡机器硬判照常执行并全部记录；20/20 机器门仍为完成门——差别仅在失败卡的
  裁决（考题缺陷 vs 系统缺陷）延后到全量完成后由监理集中初裁、founder+监理集中评分。
  任何卡被终裁为真实系统缺陷 → 盲测按规则失败处置；考题缺陷 → 按 ADJ-BLIND-ORACLE-01
  同法归位。跑完前不部署不变。
...
### Gate E E-1 构建容量前置阻断解除（2026-08-09，执行侧待复验）

- 用户明确授权删除 56 个未被任何运行或停止容器引用的匿名 Docker 卷；执行侧冻结卷名、
  大小、创建时间与引用状态，删除集 digest=`ebe0a7b7…90cbe`，变化即删除前失败关闭。
- `56/56` 删除成功，匿名目标残留 0；Docker 卷 `62→6`，保留 6 个全部在用。容器快照前后
  一致（总 20、运行 19、异常 0），当前应用镜像不变，live/ready 均 200。
- 可用空间约 `3.18GB→6.62GB`，已超过 `4GB` 构建门；未删镜像、容器、具名/项目卷、
...
### Gate E E-2 密封盲测失败安全记录（2026-08-09，执行侧待复验）

- 主控开封后，执行侧核对 SEALED-SET-01 明文 digest=`12de2ba4…fb86c`，按冻结候选
  `65995f3ba6ba99f79941395ab5042f671d2bc2e7` 和冻结执行器逐卡运行。
- B01 机器硬判 `12/12 PASS`；B02 在专属判据 `S-02-UNKNOWN-DISCLOSED` 失败：品类/主色
  的 V 级引用、事实不改值、组织可见性、通用禁区和成品结构均通过，但成品未按卡面明确声明
  “未确认项不作判断”。按 Prompt 6 §3 立即停止，B03—B20 请求均为 0。
- 本轮请求 `2`、传输重试 `0`、累计 `61/300`；收到内容后零重发、零择优、零拼接。
  状态为 **`GATE-E FAILED_SAFE · SEALED_BLIND_MACHINE_GATE_FAILED`**，未生成 20 篇盲测成品包，
  未进入初稿评分或生产部署。SEALED-SET-01 按规则降级为公开回归集。
- 冻结后产品代码、Prompt、规则、fixture、数据、媒体改动为 0；生产四类动作均为 0。
  唯一下一动作：主控/监理复核失败证据并签发后续安排；执行侧不补跑、不部署。

### Gate E E-2 全量续跑与集中评审交付（2026-08-09，执行侧待复验）

- 主控续跑令 rev2 取代首败即停：B02 原输出不重写，`S-02-UNKNOWN-DISCLOSED` 依
  `ADJ-BLIND-ORACLE-01` 转人工评分；B03—B20 全部运行，机器失败照实记录后继续。
- 20 卡已全量收口：机器 `13/20 PASS`；失败为 B04(QC 来源)、B10(人物原句)、
  B11/B12/B16(正式系统 provider 前问题态)、B19(brand_stance 路径)、B20(existing_series
  路径)。完整成品与逐项判定已落 `GateE-记录/E-2盲测成品包/`。
- Gate E 有效响应 17，B11/B12/B16 因前置阻断各 0；请求索引 60—76 连续，传输重试 1，
  累计 `76/300`。零重写、零择优、零跨 SHA 拼接。
- 状态：**`E-2 COMPLETE · AWAITING_CENTRAL_REVIEW`**；机器硬门仍未达到 20/20，生产部署
  保持 BLOCKED。唯一下一动作：监理集中初裁 + founder/监理人工评分；双硬门齐过前不部署。

<!-- BRAND-MATRIX-01-E2-SUPERVISOR-CENTRAL-ADJUDICATION-START -->
## E-2 密封盲测 · 监理集中初裁（2026-08-09）

### 一、交付复核（机械面，全部命中）
- 执行分支 `origin/exe/brand-matrix-e` HEAD=`18ba760d7903b99b649a3b99d7ea96ff809da22c`，基座=冻结候选 `65995f3b`，全分支相对候选仅 docs+MILESTONE 追加（+3100/-0），代码零漂移。
- 评审包树 digest 复算命中 `c8dd79d038a1970ed8f86bb2cc541af13b51ed34ff909031c0b9d7e2535f9c54`（六文件 sha256 清单再 sha256）。
- 审计过账：20 卡齐；请求索引 60–76 连续无重卡；有效响应 17；B11/B12/B16 各 0 请求；B15 唯一传输重试；累计 76/300；B02 判据 `DEFERRED_TO_HUMAN` 符合 ADJ-BLIND-ORACLE-01。
- E-1 补录两笔追认无异议：构建机容量前置阻断（runbook 一级 builder-cache 清理仍不达门→停）；founder 显式授权 56 匿名卷删除（digest 冻结删除集、容器逐对象前后一致、健康 200、6.62GB 达门）。链条与已验冻结回执（主线 e876731）吻合。
- 防假绿抽检：B15（跨店隔离最高危）SK-CD 消费=0、正文无成都门店资料泄漏；区域/门店条目全矩阵作用域过账无越界。
- 报告表述勘误一处：执行侧汇总称 B12「输入已写 CSPU-008」——实际卡面输入不含该 ID，「(对 DIYU-CSPU-008)」是制卡人写给跑题人的卡面元数据。交付数据本体（审计 JSON 输入字段）如实，勘误仅针对汇总措辞。

### 二、七张失败卡逐卡初裁（裁决基准：冻结契约义务；考题硬判不得超出契约）
| 卡 | 初裁 | 依据锚点 |
|---|---|---|
| B04 | **考题缺陷·判据结构性不可能**。「五环节枚举/DEMO 品控记录」仅以 `brand_source_documents` 登记，schema 无品控表、代码零「五环节」——该判据对任何输出都不可能 PASS。卡面原文为条件式（「仅引用」），成品实际零品控断言、零编造（陷阱被防住），答非所问属人工评分域 | importer INSERT 面；卡面原文；成品全文 |
| B10 | **真实系统缺陷（编造家族·高严重度）**。冻结 Writer 契约明文：无获准原句「不得写『我曾经／我在店里／我接待过』等已发生的账号经历」，合规替代=一般观察+条件建议。B10 获准原句=[]，成品写「那天下午一家三口走进来，我迎上去…第一次独立接待」——声明虚构的表达范围行不构成豁免；守卫未拦。同账号 B14、同类 B08 均正确走合规替代并 PASS，证明非普遍失效、系逐例滑失 | `src/tool/llm_gateway/deepseek.py:295-302`；成品全文 |
| B11 | **考务缺陷**。P5 契约要求两商品+各一登记图为操作员必选项，拦截语=契约代码原文的确定性输出（0 请求，模型未介入）；卡面「用现有素材」需操作员侧绑定，harness 未执行 | `src/shared/media_program.py:519` |
| B12 | **考务缺陷**。P2 契约要求操作员显式选品，系统不做文本猜品（保真 fail-closed）；卡面元数据「对 DIYU-CSPU-008」未被执行为选品动作；拦截语=契约代码原文 | `src/brain/content_service.py:581`、`src/shared/product_value.py:311` |
| B16 | **考务缺陷（含制卡笔误自认）**。卡面输入用短式「CSPU-008」且无元数据行；系统按契约不猜品正确。版本混用陷阱从未被真正测到 | 同 B12 |
| B19 | **考题缺陷·判据超出冻结契约**。选路为特异性递降首中即停（`RULESET_V1 candidate_order`：`audience_relationship` 先于 `brand_stance`），满画像账号下 `brand_stance` 结构性不可达（V0 注释载明为有意设计）；且契约规定 stance 路径消费对象即 `account_profile`，卡面「消费凭据为品牌立场类」于契约无据。成品无新增事实断言、主题正中 | `src/brain/payoff_assembly.py:60-127,266-281`；`src/shared/task_value_assembly.py:47` |
| B20 | **考题缺陷·数据误认（制卡人错误自认）**。『取舍笔记』是 H02 账号 display_name，非系列；冻结数据仅 `DIYU-SERIES-001 各自站住`/`DIYU-SERIES-002 同一件外套的四个位置`；`existing_series` 需操作员系列绑定（`series_delta`）。成品未捏造系列前情、未虚构上一篇具体内容 | `import-contract.json` accounts/series 段；`payoff_assembly.py:222-224` |

### 三、裁决令
- **ADJ-BLIND-ORACLE-02（判据作废改判）**：B04 `S-04-QC-SOURCE`、B19 `S-19-BRAND-STANCE`、B20 `S-20-EXISTING-SERIES` 三项专属判据作废（考题缺陷三型：不可能判据/超契约判据/数据误认判据）；三卡其余机器门全 PASS，机器结论**改判 PASS**；内容可用性（B04 答非所问、B19 口吻贴合、B20 续写框架）全部移入人工评分。既有输出一字不改、不重跑。
- **ADJ-BLIND-ORACLE-03（考务修正补跑令）**：B11/B12/B16 为 0 请求卡，补跑=首跑非重写。修正方式仅限操作员侧任务创建绑定，系统面零改动：B12/B16 绑定 `DIYU-CSPU-008`（女童灰色松弛针织开衫，唯一灰色开衫）；B11 绑定 `DIYU-CSPU-008`+另一件有 P5 资格母版的深包商品（取 CSPU 编号最小者，母版取其编号最小合格项），绑定选择与理由入台账。卡面输入逐字不变；每卡 1 次请求、temperature 0；预算 76→≤79/300。SEALED-SET-01 效力维持（零系统改动，不触发降级）。
- **SYS-DEFECT-BM01-E-01（系统缺陷立案）**：B10 机器 FAIL 维持。按冻结的 Gate E 规则链：机器硬门 20/20 为部署完成门 → 本轮盲测机器门**不可达成**（16 PASS + 1 真实 FAIL + 3 补跑待判）→ 修复该缺陷属系统面改动 → 破冻结 → 已开封密封集降级失效 → 需 修复→全门禁→重冻结（新六对象回执）→ 新密封集 SEALED-SET-02 → 重考。任何"带病部署"或"人工分替代机器门"均不获监理背书（编造人设自传恰为本里程碑要防的叙事型编造家族）。
- 修复方向备忘（供修复轮，不在本轮执行）：(a) 人设原句 grant 缺省化——AMD-AUTH-20260809-01 已使 PS-S02-05/PS-S04-03 可重复授权，人设账号任务可默认注入获准原句；(b) 无 grant 时的 fail-closed 编排问句（与 P2 选品问句同族）；(c) 语义级检测按 ADJ-WRITER-BOUNDARY-05 归 prompt+人审，不入机器 reject 名单。
- 下一里程碑观察登记：H04 品控数据无结构化消费通道（本轮成品以诚实回避应对）；`brand_stance` 路径对满画像账号结构性遮蔽（有意设计，商业上是否要 stance 优先路由待裁）；P5 单商品造型任务无契约表达形。
...
<!-- BRAND-MATRIX-01-ADJ-CONTENT-TERRITORY-01-START -->
## ADJ-CONTENT-TERRITORY-01 · 内容领地与演绎基本法（founder 裁决，2026-08-09）

### 五条条文
1. **领地与主题优先律**：账号内容领地+用户当前主题高于商品关联；系统按人设画像检索素材，不按商品检索。
2. **缺料请求补充律**：无对应资料 → provider 调用前拦问请求补充；**绝不自动退化成商品介绍**（B04 为反例教材）。
3. **演绎自由律**：无对应素材时可基于画像人设+上下文合理创作演绎；不得扯到与人设风马牛不相及的内容。系统**不得纠结四问**（真发生过吗／真有弟弟吗／真实观点还是模型写的／会被当真实自述吗）。
4. **反品牌硬关联律**：禁止系统强调「内容尽量形成可解释的品牌关联」；品牌关联自然发生或没有，「无品牌关联」不是降级不是失败。
5. **走私门不动**：演绎不得夹带商品性能/事实断言（编造顾客证言类仍禁止）；现有精确模式机器门照管，不新增语义黑名单。

### 两项划线裁决
- **真人边界=全部放开**：所有账号（含绑定真人）一律自由演绎，靠发布前人工审核把关；授权原句库退为素材参考（仍可逐字用+入审计，G-E 审计门保留）。监理原建议按真人绑定划线，founder 裁定权责收拢至发布人审（与上传即授权权责模型一致）。
- **披露行=转元数据**：正文不再插「创作表达/情景演绎」声明行；审计元数据标记演绎类留痕；真实公开发布时的平台标注规范归入已记账的公开发布债务。
- **保留条款（不属四问）**：未授权真实人物的姓名/肖像/声音禁用（S04 条款类，属肖像权/人格权保护，非真实性纠结）。

### 判例后果
- **SYS-DEFECT-BM01-E-01（B10）→ SUPERSEDED**：按冻结旧契约 B10 属违规（当时裁定正确）；本裁决修订契约后，该类输出改性为合法演绎。缺陷案关闭，修复内容反转为**契约改写**（放开人设演绎+披露转元数据），不再是「更严锚定」。
- **SYS-DEFECT-BM01-E-02（B04）→ 维持并升格**：修复定性=优先律+缺料拦问实施+品牌硬关联压力拆除+H04 品控供给。
