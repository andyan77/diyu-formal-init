...
def render_display_workbench(
    ...
    content = f"<a href='/'>返回应用首页</a><h1>陈列搭配（对内）</h1>{identity_text}<p>DM01 确定性陈列编译：根据本次库存、门店挂杆结构和本次表达生成参考建议。</p>{_notice(notice)}<form method='post' action='/ui/display/generate'><textarea name='inventory_text' required placeholder='今天这组墙可用：商品编号 3 件……'></textarea><button>生成墙面方案</button></form>{artifact}"
