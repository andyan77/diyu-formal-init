...
def parse_observation(value: object) -> ReviewerObservation:
    ...
    return ReviewerObservation(
        ...
        people=_string_tuple(value.get("people")),
        relationships=_string_tuple(value.get("relationships")),
        actions_or_events=_string_tuple(value.get("actions_or_events")),
        dialogue=_string_tuple(value.get("dialogue")),
        motives=_string_tuple(value.get("motives")),
        causes=_string_tuple(value.get("causes")),
        results=_string_tuple(value.get("results")),
        times=_string_tuple(value.get("times")),
        locations=_string_tuple(value.get("locations")),
        possessions=_string_tuple(value.get("possessions")),
    ...
    *,
    ...
    for scene_id, declared_resources in scene_resource_refs.items():
        ...
        linked_types = {
            block.block_type
            for block in blocks
            if scene_id in block.scene_ids
        }
